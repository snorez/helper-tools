...
class cve_class(object):
        def __init__(self, id="", desc="", url="", status="TODO"):
                self.id = id
                self.desc = desc
                self.url = url
                self.status = status
...
def get_cve_lists(keyword="", outfile=""):
        ...
        for item in cve_list:
                ...
                this_json["CVE_STATUS"] = item.status
                this_json["CVE_DESC"] = item.desc
                cve_json.append(this_json)
        with open(outfile, "w") as f:
                f.write(json.dumps(cve_json, indent=4))
                print("(%d)JSON format content has been written to %s"%(len(cve_json), outfile))
