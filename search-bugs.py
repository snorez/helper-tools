...
class bugs_gpz_class(object):
        def __init__(self, id="", date="", summary="", url="", author="", progress="TODO"):
                self.id = id
                self.date = date
...
def get_bugs_lists(keywords="", outfile=""):
        ...
        while True:
                ...
                for item in bugs_list:
                        ...
                        this_json["BUGS_PROGRESS"] = item.progress
                        insert(bugs_json, this_json, "BUGS_ID")
        ...
        with open(outfile, "w") as f:
                ...
                print("(new:%d, total:%d)has been written to %s"%(len(bugs_json)-orig_len, len(bugs_json), outfile))
